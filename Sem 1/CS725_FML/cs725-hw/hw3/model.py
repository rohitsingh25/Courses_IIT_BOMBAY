...
class NaiveBayes:
    def fit(self, X, y):
        ...
        for c in self.classes:
            ...
            self.gaussian[str(c)] = {
                "mean": np.mean(X_c[:2], axis=0),
                "variance": np.var(X_c[:2], axis=0)
            }
            p_x3 = np.mean(X_c[:, 2])
            p_x4 = np.mean(X_c[:, 3])
            self.bernoulli[str(c)] = [p_x3, p_x4]
            mu_x5 = np.median(X_c[:, 4])
            mu_x6 = np.median(X_c[:, 5])
            b_x5 = np.mean(np.abs(X_c[:, 4] - mu_x5))
            b_x6 = np.mean(np.abs(X_c[:, 5] - mu_x6))
            self.laplace[str(c)] = [mu_x5, mu_x6, b_x5, b_x6]
